---
title: "Begging costs rather than food received cause brood size effect on growth in zebra finches"
...
zf <- read_excel("ZF_Begging_Database_Final.xlsx", 
                 sheet="Behavior")
...
chicks <- zf %>%
  ...
  group_by(age, broodid, nest, frequency, behavior, proportion)
...
w <- read_excel("ZF_Begging_Database_Final.xlsx", 
                           sheet="Mass")

#insert data for average day at cross-fostering
a <- read_excel("ZF_Begging_Database_Final.xlsx", 
                         sheet = "AvgAgeCF")
...
mg_plot<- ggplot(weight_c_d, aes(x = Nest, y =weight_gain_day, fill=Nest))+
  ...
  annotate(geom="text", x=2, y=1.22, label="42", size=6)+
...
w15_plot<- ggplot(weight_c15, aes(x = Nest, y=Weight, fill=Nest))+
  ...
  annotate(geom="text", x=2, y=16.5, label="42", size=6)+
  stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), color = "black", linetype = "dashed", size = 0.6)+
  scale_x_discrete(labels = c("2" = "small", "6" = "large")) 
w15_plot
...
tiff("Fig1.tiff", units="in", width=9, height=6, res=300)
ggarrange(mg_plot, w15_plot, nrow=1, ncol=2, align="h", labels = c("(a)", "(b)"),common.legend = T, legend = "right",label.x = c(0.16,0.16),label.y= c(0.98,0.98))
dev.off()
...
# Fig.S1 ------------------------------------------------------------------
tiff("FigS1.tiff", units="in", width=8, height=6, res=300)
...
bc<- ggplot(begging, aes(x = category, y=proportion^0.5, fill=nest))+
  geom_violin(alpha=0.5, position = position_dodge(width = .75),size=1,color=NA) +
  ...
  guides(fill = guide_legend(override.aes = list(alpha = 0.7,color="black")))+
  annotate(geom="text", x=0.66, y=0.75, label="N=6", size=4) + 
  annotate(geom="text", x=1.12, y=0.75, label="22", size=4)+ 
  annotate(geom="text", x=1.8, y=0.75, label="32", size=4)+
  annotate(geom="text", x=2.2, y=0.75, label="32", size=4)+
  annotate(geom="text", x=2.8, y=0.75, label="26", size=4)+
  annotate(geom="text", x=3.2, y=0.75, label="49", size=4)+
  annotate(geom="text", x=3.8, y=0.75, label="20", size=4)+
  annotate(geom="text", x=4.2, y=0.75, label="12", size=4)
bc

tiff("Fig3.tiff", units="in", width=6, height=6, res=300)
bc
dev.off()
...
#merge male and female from the dataset of 2023
parents <- zf %>%
  filter(subject %in% c("female", "male")) %>%
  group_by(broodid, nest, individual, subject, behavior, proportion)

tabyl(parents$nest)

parents <- subset(parents, behavior != "begging")
feeding_p <- subset(parents, behavior == "feeding")


#calculate feeding proportion of parents by feeding proportion of chicks (2012,2022)
#broods of 2=sum proportion
#broods of 6=sum proportion *3
modified_feeding_c <- feeding_c %>%
  group_by(broodid, observer,age,nest,category) %>%
  summarise(avg_proportion = ifelse(any(nest == 2), mean(proportion[nest == 2]) * 2 / 2, 
                                    ifelse(any(nest == 6), mean(proportion[nest == 6]) * 6 / 2, NA)))


hist(modified_feeding_c$avg_proportion)
...
tiff("Fig4.tiff", units="in", width=9, height=6, res=300)
ggplot(modified_feeding_c, aes(x = category, y = avg_proportion^0.5, fill=nest))+
  geom_violin(alpha=0.5, position = position_dodge(width = .75),size=1,color=NA) +
  ...
  theme(
    ...
    legend.key.size =unit(0.8, "cm"))+
  labs(x = "Age (days)",
       y = expression(sqrt("proportion of time parents spent feeding chicks")),
       fill = "Brood size")+
  ...
  annotate(geom="text", x=3.8, 0.7, label="10", size=5)+
...
modified_feeding_c %>%
  group_by(nest) %>%
  summarise(mean=mean(modified_proportion_2))
...
tiff("Fig5.tiff", units="in", width=9, height=6, res=300)
ggplot(feeding_c, aes(x = category, y =proportion^0.5, fill=nest))+
  ...
  annotate(geom="text", x=3.8, y=0.32, label="20", size=5)+
...
# feeding parents ---------------------------------------------------------
modified_feeding_c$trans_per<- asin(sqrt(modified_feeding_c$avg_proportion/100))
...
#find average proportion of time chicks spent feeding in each brood size
feeding_c %>%
  group_by(nest) %>%
